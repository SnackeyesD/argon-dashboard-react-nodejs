...
export default class Product {
    ...
    static async create(productData) {
        ...
        if(!id_vendeur) {
           //throw new Error('ID vendeur est requis pour créer un produit')
           console.log("Ce produit n'a pas de vendeur associé");
        }
        try {
            const [result] = await pool.execute(`
                INSERT INTO produits (id_vendeur, id_categorie, nom_produit, description, prix, stock, statut, sku)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?)
            `, [id_vendeur || 1, id_categorie, nom_produit, description, prix, stock, 'actif', sku]);
    ...
    // Pour rechercher les catégories des produits
    static async getCategories() {
        try {
            const [rows] = await pool.execute(`
                SELECT id_categorie, nom_categorie FROM categories where active = 1 ORDER BY ordre_affichage ASC
                `)
                return rows;
        } catch (error) {
